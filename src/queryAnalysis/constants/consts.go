...
SELECT st.text AS sql_text, qp.query_plan AS query_plan_xml, 
...
COALESCE(n.value('(@MemoryGrantInfo/*[local-name()="GrantedMemoryKb"])[1]', 'INT'), 0) AS GrantedMemoryKb, 
COALESCE(n.value('(@Warnings/*[local-name()="Warnings"]/*[local-name()="SpillOccurred"])[1]', 'BIT'), 0) AS SpillOccurred, 
COALESCE(n.value('(@Warnings/*[local-name()="Warnings"]/*[local-name()="NoJoinPredicate"])[1]', 'BIT'), 0) AS NoJoinPredicate, 
qs.total_worker_time, qs.total_elapsed_time, qs.total_logical_reads, qs.total_logical_writes, qs.execution_count
