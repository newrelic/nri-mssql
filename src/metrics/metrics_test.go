...
func (b *mockConnectionBuilder) expectMemoryAndDiskQueries(withErrors bool, withPartialErrors bool) *mockConnectionBuilder {
	// Disk Space
	diskQuery := b.mock.ExpectQuery(`^SELECT\s+DB_NAME\(\)\s+AS\s+db_name,\s+CAST\(DATABASEPROPERTYEX\(DB_NAME\(\),\s+'MaxSizeInBytes'\)\s+AS\s+BIGINT\)\s+AS\s+total_disk_space;?$`)
	if withErrors || withPartialErrors {
		diskQuery.WillReturnError(errors.New("disk space error"))
	} else {
		diskQuery.WillReturnRows(sqlmock.NewRows([]string{"db_name", "total_disk_space"}).AddRow(b.dbName, 104857600))
	}
	...
	if withErrors {
		utilQuery.WillReturnError(errors.New("utilization error"))
	} else {
	...
	if withErrors || withPartialErrors {
		totalMemQuery.WillReturnError(errors.New("total memory error"))
	} else {
...
func Test_populateDatabaseMetrics(t *testing.T) {
	...
	testCases := []DatabaseMetricsTesCase{
		...
		{
			name: "Engine edition 5: collecting partial memory metrics",
			setupMock: func(mock sqlmock.Sqlmock) {
				databaseRows := sqlmock.NewRows([]string{"db_name"}).
					AddRow("db-1").
					AddRow("db-2")
