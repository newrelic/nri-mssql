...
func TestValidatePreConditions(t *testing.T) {
	...
	result := ValidatePreConditions(sqlConnection, nil)
...
func TestValidatePreConditions_DMVOnlyMode_SQL2016(t *testing.T) {
	...
	// Create arguments with DMV-only mode enabled
	arguments := &args.ArgumentList{
		QueryMonitoringDisableHistoricalInformation: true,
	}
	...
	result := ValidatePreConditions(sqlConnection, arguments)
...
func TestValidatePreConditions_QueryStoreMode_SQL2016(t *testing.T) {
	...
	// Create arguments with Query Store mode (default, flag = false)
	arguments := &args.ArgumentList{
		QueryMonitoringDisableHistoricalInformation: false,
	}
...
func TestValidatePreConditions_NilArguments(t *testing.T) {
	sqlConnection, mock := setupMockDB(t)
	defer sqlConnection.Connection.Close()

	// Mock SQL Server 2017 version (should pass with nil arguments = Query Store mode)
	mock.ExpectQuery(regexp.QuoteMeta(getSQLServerVersionQuery)).
		WillReturnRows(sqlmock.NewRows([]string{"@@VERSION"}).
	...
	result := ValidatePreConditions(sqlConnection, nil)
	assert.True(t, result, "Nil arguments should be handled gracefully and default to Query Store mode")
	assert.NoError(t, mock.ExpectationsWereMet())
}
