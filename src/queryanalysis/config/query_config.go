...
var QueriesWithHistoricalInformation = []models.QueryDetailsDto{
	{
...
WITH AggregatedStats AS (
    SELECT
        ...
        MIN(qt.objectid) AS objectid,                -- For schema name lookup
...
SELECT TOP (@Limit)
    ...
    s.min_elapsed_time / 1000.0 AS min_elapsed_time_ms,
...
var Queries = []models.QueryDetailsDto{
	...
	{
		EventName: "MSSQLWaitTimeAnalysis",
		Query: `-- Optimized query for current waiting sessions with proper filtering and sorting
				SELECT TOP 1000
					r.session_id,
